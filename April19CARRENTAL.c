...
check = fopen("crypto_state.txt", "a");
if (check != NULL) fclose(check);
...
void initializeCars(Car garage[CATEGORIES][SLOTS]) {
    ...
    // ? Check if data already exists (DO NOT overwrite loaded data)
    ...
    if (hasData == 1) {
        return; // Exit if cars already loaded from file
    }

    // ? Clear memory (ensures no garbage values)
    for (i = 0; i < CATEGORIES; i++) {
        for (j = 0; j < SLOTS; j++) {
            garage[i][j] = (Car){0};
        }
